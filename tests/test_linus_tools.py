...
# -*- coding: utf-8 -*-
"""
🏛️ TEMPLE IAM - Tests linus_tools.py
⚡ Coverage target: 100%
...
class TestRepairLlmFilePath:
    ...
    def test_dispatch_write_accepts_garbled_drive(self, tmp_path):
        ...
        r = dispatch_tool(
            "write_file",
            {"file_path": garbled, "content": "linus_planner.py\n"},
            cwd=str(tmp_path),
        )
        assert r.success, r.error
        assert (scratch / "found.txt").read_text(encoding="utf-8").startswith("linus_planner")
...
class TestToolBashCwd:
    ...
    @patch("linus_tools.platform.system", return_value="Linux")
    @patch("linus_tools.subprocess.run")
...
class TestNormalizeCommand:
    ...
    def test_grep_command_preserved(self):
        assert _normalize_command("bash grep -c \"^def \" linus_tools.py") == "grep -c \"^def \" linus_tools.py"
...
class TestToolResult:
    ...
    def test_redact_returns_new_instance_leaves_original(self):
        from linus_tools import _redact_result
...
class TestProtectedReferenceFiles:
    ...
    def test_env_allows_reference_overwrite(self, tmp_path, monkeypatch):
        ref = tmp_path / "mini_pytest.py"
        ref.write_text("print('reference')", encoding="utf-8")
        monkeypatch.setenv("LINUS_ALLOW_REFERENCE_OVERWRITE", "1")
...
class TestToolBash:
    ...
    @patch("linus_tools.platform.system", return_value="Windows")
    @patch("linus_tools.subprocess.run")
    ...
    @patch("linus_tools.platform.system", return_value="Linux")
    @patch("linus_tools.subprocess.run", side_effect=subprocess.TimeoutExpired("cmd", 5))
    ...
    @patch("linus_tools.platform.system", return_value="Linux")
    @patch("linus_tools.subprocess.run", side_effect=FileNotFoundError("no shell"))
    ...
    @patch("linus_tools.platform.system", return_value="Linux")
    @patch("linus_tools.subprocess.run", side_effect=OSError("unexpected"))
...
class TestToolGlob:
    ...
    def test_glob_exception_returns_failure(self):
        with patch("linus_tools.Path.glob", side_effect=ValueError("bad pattern")):
            r = tool_glob("**/*.py", "/tmp")
        assert r.success is False
...
class TestToolGrep:
    ...
    def test_grep_unreadable_file_skipped(self, tmp_path):
        ...
        f.write_text("match")
        with patch("linus_tools.Path.read_text", side_effect=PermissionError("denied")):
            r = tool_grep("match", str(tmp_path))
        assert r.success is True
        assert "(no matches)" in r.output

    def test_grep_outer_exception_returns_failure(self, tmp_path):
        with patch("linus_tools.Path.resolve", side_effect=OSError("broken")):
...
class TestToolWebFetch:
    ...
    def _allow_urls(self):
        ...
        with patch("linus_tools._validate_and_pin_url", side_effect=_allow):
            yield

    @patch("linus_tools.requests.get")
    ...
    def test_download_capped_at_max_bytes(self, mock_get):
        ...
        from linus_tools import MAX_FETCH_BYTES
...
class TestIsBlockedUrl:
    ...
    def test_unparseable_blocked(self):
        with patch("linus_tools.urlparse", side_effect=ValueError("boom")):
            assert _is_blocked_url("http://x")[0] is True

    def test_cloud_metadata_blocked(self):
        with patch("linus_tools.socket.getaddrinfo", return_value=_addrinfo("169.254.169.254")):
            blocked, reason = _is_blocked_url("http://metadata.evil/latest/meta-data/")
        assert blocked is True
        assert "SSRF" in reason

    def test_loopback_blocked(self):
        with patch("linus_tools.socket.getaddrinfo", return_value=_addrinfo("127.0.0.1")):
            assert _is_blocked_url("http://localhost:8080/admin")[0] is True

    def test_private_10_blocked(self):
        with patch("linus_tools.socket.getaddrinfo", return_value=_addrinfo("10.0.0.5")):
            assert _is_blocked_url("http://internal.svc/")[0] is True

    def test_private_192_blocked(self):
        with patch("linus_tools.socket.getaddrinfo", return_value=_addrinfo("192.168.1.1")):
            assert _is_blocked_url("http://router.local/")[0] is True

    def test_public_allowed(self):
        with patch("linus_tools.socket.getaddrinfo", return_value=_addrinfo("93.184.216.34")):
            blocked, reason = _is_blocked_url("https://example.com/page")
        assert blocked is False
        assert reason == ""

    def test_unresolvable_blocked(self):
        with patch("linus_tools.socket.getaddrinfo", side_effect=socket.gaierror("nope")):
    ...
    def test_invalid_ip_in_addrinfo_skipped(self):
        ...
        with patch("linus_tools.socket.getaddrinfo", return_value=infos):
...
class TestWebFetchSSRF:
    ...
    def test_fetch_public_allowed(self):
        with patch("linus_tools.socket.getaddrinfo", return_value=_addrinfo("93.184.216.34")):
            with patch("linus_tools.requests.get",
...
class TestDispatchTool:
    def test_dispatch_bash(self):
        with patch("linus_tools.tool_bash") as mock:
    ...
    def test_dispatch_read_file(self):
        with patch("linus_tools.tool_read_file") as mock:
    ...
    def test_dispatch_edit_file(self):
        with patch("linus_tools.tool_edit_file") as mock:
            mock.return_value = ToolResult(success=True, output="ok")
    ...
    def test_dispatch_grep(self):
        with patch("linus_tools.tool_grep") as mock:
            mock.return_value = ToolResult(success=True, output="ok")
    ...
    def test_dispatch_web_fetch(self):
        with patch("linus_tools.tool_web_fetch") as mock:
            mock.return_value = ToolResult(success=True, output="ok")
    ...
    def test_dispatch_generic_exception(self):
        with patch("linus_tools.tool_bash", side_effect=RuntimeError("unexpected")):
...
class TestToolBraveSearch:
    def test_returns_formatted_results(self):
        with patch("linus_tools.requests.get", return_value=_brave_ok()):
    ...
    def test_multiple_results_numbered(self):
        ...
        with patch("linus_tools.requests.get", return_value=_brave_ok(results)):
    ...
    def test_count_clamped_to_max(self):
        with patch("linus_tools.requests.get", return_value=_brave_ok()) as mock_get:
    ...
    def test_missing_api_key_returns_failure(self):
        ...
        import linus_tools
        orig = linus_tools.BRAVE_API_KEY
        linus_tools.BRAVE_API_KEY = None
        try:
            r = tool_brave_search("q", api_key=None)
        finally:
            linus_tools.BRAVE_API_KEY = orig
        assert r.success is False
    ...
    def test_timeout_returns_failure(self):
        import requests as _req
        with patch("linus_tools.requests.get", side_effect=_req.Timeout()):
            r = tool_brave_search("q", api_key="fake-key")
        assert r.success is False
        assert "Timeout" in r.error
    ...
    def test_generic_exception_returns_failure(self):
        with patch("linus_tools.requests.get", side_effect=ValueError("bad json")):
            r = tool_brave_search("q", api_key="fake-key")
        assert r.success is False
    ...
    def test_dispatch_brave_search(self):
        with patch("linus_tools.tool_brave_search") as mock:
...
class TestValidateAndPinEdges:
    ...
    def test_resolves_to_no_valid_ip_blocked(self):
        with patch("linus_tools.socket.getaddrinfo",
