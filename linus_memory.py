...
# -*- coding: utf-8 -*-
"""
🏛️ TEMPLE IAM - LINUS MEMORY
⚡ Mémoire persistante inter-sessions pour l'agent LINUS
🔥 Charge au démarrage · Sauvegarde à la fin · Markdown lisible

Copyright © 2024 Temple IAM - All Rights Reserved

Architecture:
    Fichier .linus_memory.md stocké dans le cwd de la tâche.
...
# ── Constantes ────────────────────────────────────────────────────────────────
DEFAULT_MEMORY_FILE = ".linus_memory.md"
MEMORY_HEADER       = "# 🧠 LINUS Memory"
ENTRY_SEPARATOR     = "\n---\n"
...
def resolve_memory_path(
    ...
    Priorité :
        ...
        2. cwd/.linus_memory.md (si cwd fourni)
        3. ./.linus_memory.md (répertoire courant)
...
# ── Compat linus_hybrid (mémoire conversationnelle en session) ───────────────
...
_memory_singleton: Optional[LinusMemory] = None


def get_memory() -> LinusMemory:
    global _memory_singleton
    if _memory_singleton is None:
        _memory_singleton = LinusMemory()
...
def resolve_reference(question: str, mem: LinusMemory) -> Optional[str]:
