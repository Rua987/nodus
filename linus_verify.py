...
Idée :
    Tant que LINUS fait confiance à la CLAIM du modèle, un modèle faible ou
...
# Normalisation déterministe des NOMS d'outils produits par le planificateur LINUS
...
def task_requests_write(task: str) -> bool:
    ...
    Utilise pour completer un plan LINUS qui a omis write_file.
    Fonction pure.
    """
    if not task or not task.strip():
        return False
...
def is_valid_basename_py(text: str) -> bool:
    ...
    Fonction pure.
    >>> is_valid_basename_py("linus_planner.py")
    True
