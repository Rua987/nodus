...
# -*- coding: utf-8 -*-
"""
LINUS Vibe — session terminal type Claude Code.

Boucle interactive multi-tours sur le VRAI harness (linus_agent + gates Fable):
  - stream_agent : outils visibles en direct (round / result / verify)
  - GLM 5.2 cloud par defaut, --verify, --text-tools si OpenRouter
  - memoire session (.linus_memory.md), cwd = repertoire courant

Usage:
    python linus_vibe.py
    python linus_vibe.py --cwd C:\\Users\\admin\\linus_sandbox
    python linus_vibe.py -m openrouter/openai/gpt-oss-120b:free

PowerShell (profil) :
    function linus-vibe { python ...\\linus_vibe.py --cwd "$PWD" @args }
"""
...
from linus_agent import OPENROUTER_CLOUD_MODEL, stream_agent  # noqa: E402
from linus_chat import EXIT_COMMANDS, handle_command, parse_input  # noqa: E402
from linus_memory import resolve_memory_path  # noqa: E402

PROMPT = "linus> "

BANNER = """\
LINUS Vibe — coding agent (Claude Code style)
...
def handle_vibe_command(
    ...
    if command == "/help":
        from linus_chat import help_text
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="linus_vibe", description="LINUS interactive vibe coding session")
